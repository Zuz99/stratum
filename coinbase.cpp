...
void coinbase_aux(YAAMP_JOB_TEMPLATE *templ, char *aux_script)
{
	...
	memset(merkle_hash, 0, sizeof(merkle_hash));
	// hashlist[0] is already the chain merkle root as raw bytes (hex).
	// Insert it as-is after the fabe6d6d marker.
	strncpy(merkle_hash, hashlist[0].c_str(), sizeof(merkle_hash)-1);

	// AuxPoW marker: fabe6d6d + chain merkle root (32 bytes) +
	// merkle tree size (LE32) + nonce (LE32)
	char size_le[16] = {0};
	char nonce_le[16] = {0};
	uint32_t nSize = (uint32_t)templ->auxs_size;
	uint32_t nNonce = (uint32_t)templ->aux_nonce;
	sprintf(size_le, "%02x%02x%02x%02x",
		nSize & 0xff, (nSize >> 8) & 0xff, (nSize >> 16) & 0xff, (nSize >> 24) & 0xff);
	sprintf(nonce_le, "%02x%02x%02x%02x",
		nNonce & 0xff, (nNonce >> 8) & 0xff, (nNonce >> 16) & 0xff, (nNonce >> 24) & 0xff);

	sprintf(aux_script+strlen(aux_script), "fabe6d6d%s%s%s", merkle_hash, size_le, nonce_le);
...
void coinbase_create(YAAMP_COIND *coind, YAAMP_JOB_TEMPLATE *templ, json_value *json_result)
{
	...
	// Kylacoin (flex) uses a different block version
	if(!strcmp(g_stratum_algo, "flex"))
	...
	// Pool tag (ASCII -> hex). Default can be overridden via STRATUM:coinbase_tag.
	char script2[4*1024] = {0};
	for(size_t i=0; g_coinbase_tag[i] && strlen(script2) < sizeof(script2)-3; i++) {
		char tmp[3];
		sprintf(tmp, "%02x", (unsigned char) g_coinbase_tag[i]);
		strcat(script2, tmp);
	}
