...
void db_update_coinds(YAAMP_DB *db)
{
	...
	while((row = mysql_fetch_row(result)) != NULL)
	{
		...
		if(row[36]) coind->usemweb = atoi(row[36]) > 0;

		// global flag used in a few places (kept for backward compatibility)
		if(coind->usesegwit || coind->usemweb) g_stratum_segwit = true;
