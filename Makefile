...
# Many operators compile on one host and deploy on another (different CPU features).
# -march=native can emit instructions (AVX/AES/etc) that crash with "Illegal instruction".
# Keep stratum binaries portable.
CFLAGS= -g -O2
...
projectcode1:
	@# When building from a source tar/zip without .git (or without git installed),
	@# skip submodule init/update and just build what is already present.
