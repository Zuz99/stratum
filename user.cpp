...
//////////////////////////////////////////////////////////////////////////////////////
// Dual wallet / AuxPoW merged mining: store per-user secondary payout addresses.
// Miner password format (as documented in web/gettingstarted.php):
//   aw=SYMBOL=WALLET[/SYMBOL2=WALLET2/...]

static int db_get_aux_coinid(YAAMP_DB *db, const char *symbol)
{
	...
	string_upper(sym);
	db_check_user_input(sym);

	// Only accept AuxPoW coins (merged mining) for this stratum algo.
	// This keeps the wallet table limited to coins this stratum instance can actually merge-mine.
	char query[1024];
	snprintf(query, sizeof(query),
		"SELECT id FROM coins WHERE installed AND auxpow AND algo='%s' AND (symbol='%s' OR symbol2='%s') LIMIT 1",
		g_stratum_algo, sym, sym);
	...
	int coinid = 0;
	MYSQL_ROW row = mysql_fetch_row(result);
	if(row && row[0]) coinid = atoi(row[0]);
	mysql_free_result(result);
	return coinid;
}

static void db_store_aux_wallets(YAAMP_DB *db, YAAMP_CLIENT *client)
{
	if(!db || !client) return;
	if(client->userid <= 0) return;
	if(!client->password[0]) return;

	const char *aw = strstr(client->password, "aw=");
	if(!aw) return;
	aw += 3;

	// Copy until the next parameter separator (comma)
	char buf[512] = {0};
	strncpy(buf, aw, sizeof(buf)-1);
	...
	while(entry && count < 16)
	{
		...
		*eq = '\0';
		...
		char sym[16] = {0};
		strncpy(sym, entry, sizeof(sym)-1);
		string_upper(sym);
		db_check_user_input(sym);

		char address[129] = {0};
		strncpy(address, eq+1, sizeof(address)-1);
		db_check_user_input(address);

		if(strlen(sym) < 2 || strlen(sym) > 12 || !address[0]) {
			entry = strtok_r(NULL, "/", &saveptr);
			continue;
		}

		int coinid = db_get_aux_coinid(db, sym);
		if(coinid > 0)
		{
			db_query(db,
				"INSERT INTO account_wallets (account_id, coinid, address, created, updated) "
				"VALUES (%d,%d,'%s',%d,%d) "
				"ON DUPLICATE KEY UPDATE address='%s', updated=%d",
				client->userid, coinid, address, now, now, address, now);
		}
...
void db_add_user(YAAMP_DB *db, YAAMP_CLIENT *client)
{
	...
	else if(client->userid == 0 && strlen(client->username) >= MIN_ADDRESS_LEN)
	{
		...
		// Store optional secondary payout addresses for AuxPoW merged mining.
		// Example: -p c=LTC,aw=DOGE=DOGE_WALLET/BELLS=BELLS_WALLET
		db_store_aux_wallets(db, client);
}
