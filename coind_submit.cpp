...
bool coind_submitgetauxblock(YAAMP_COIND *coind, const char *hash, const char *block)
{
	...
	// Prefer "getauxblock" (Dogecoin-style). If not available, fallback to "submitauxblock" (Bitcoin-Core AuxPoW).
	...
	if(json)
	{
		...
		if(json_error && json_error->type != json_null)
		{
			...
			{
				json_value_free(json);
				json = rpc_call(&coind->rpc, "submitauxblock", params);
			}
			else
			{
					const char *p = json_get_string(json_error, "message");
	...
	{
		// If the daemon doesn't like/know getauxblock, try submitauxblock.
		json = rpc_call(&coind->rpc, "submitauxblock", params);
	}
	...
	if(json_result && json_result->type == json_boolean && !json_result->u.boolean)
		stratumlog("AUX REJECT %s returned false for %s\n", coind->name, hash);
