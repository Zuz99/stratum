...
{
	int count = 0;
	char buffer[128*1024] = "insert into blocks (height, blockhash, coin_id, userid, workerid, category, solo, difficulty, difficulty_user, time, algo, segwit) values ";
	...
	for(CLI li = g_list_block.first; li; li = li->next)
	{
		...
		if(count) strcat(buffer, ",");
		sprintf(buffer+strlen(buffer), "(%d, '%s', %d, %d, %d, 'new', 0, %f, %f, %d, '%s', %d)",
