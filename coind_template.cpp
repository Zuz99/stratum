...
static bool auxpow_target_from_bits_be(const char* bits_hex, char* out_target_hex, size_t outsz)
{
	// Lyncoin (and many AuxPoW daemons) expose only "bits" and a deprecated
	// "_target" that is in reversed byte order. For comparisons we always
	// keep a 32-byte big-endian target hex.
	if(!bits_hex || !bits_hex[0] || !out_target_hex || outsz < 65) return false;
	unsigned char target_be[32];
	compact_to_target_be(bits_hex, target_be);
	char hex[65];
	memset(hex, 0, sizeof(hex));
	hexlify(hex, target_be, 32);
	hex[64] = '\0';
	strncpy(out_target_hex, hex, outsz-1);
	out_target_hex[outsz-1] = '\0';
	return true;
}
...
void coind_getauxblock(YAAMP_COIND *coind)
{
	...
	strncpy(oldhash, coind->aux.hash, sizeof(oldhash)-1);

	// Prefer the Dogecoin-style "getauxblock" RPC.
	...
	if(json)
	{
		...
		{
			int code = (int)json_get_int(json_error, "code");
			// -32601: Method not found -> fallback to Bitcoin-Core AuxPoW RPCs.
			if(code != -32601)
			{
		...
		else
		{
			...
			{
				coind->aux.chainid = (int)json_get_int(json_result, "chainid");

				const char *p = json_get_string(json_result, "target");
				if(p) strncpy(coind->aux.target, p, sizeof(coind->aux.target)-1);

				p = json_get_string(json_result, "hash");
				if(p) strncpy(coind->aux.hash, p, sizeof(coind->aux.hash)-1);

				if(oldhash[0] && strcmp(oldhash, coind->aux.hash) != 0)
					stratumlog("auxpoll: new %s block %s\n", coind->symbol, coind->aux.hash);

				json_value_free(json);
				return;
			}

			// Unexpected format -> fallback to createauxblock.
			json_value_free(json);
			json = NULL;
		}
	}

	// Fallback for Bitcoin-Core style AuxPoW coins: createauxblock/submitauxblock.
	if(!coind->wallet[0])
	{
		stratumlog("ERROR %s auxpow requires a payout address (coind->wallet empty)\n", coind->name);
		coind_error(coind, "coind_createauxblock");
		return;
	}

	char params[1200];
	snprintf(params, sizeof(params), "[\"%s\"]", coind->wallet);

	json = rpc_call(&coind->rpc, "createauxblock", params);
	if(!json)
	{
		coind_error(coind, "coind_createauxblock");
		return;
	...
	const char *p = json_get_string(json_result, "hash");
	if(p) strncpy(coind->aux.hash, p, sizeof(coind->aux.hash)-1);

	// target can be provided directly, or as "_target" reversed-endian (deprecated), or only "bits".
	// We always normalize coind->aux.target to big-endian 32-byte hex.
	p = json_get_string(json_result, "target");
	if(p && strlen(p) >= 64)
	{
		strncpy(coind->aux.target, p, sizeof(coind->aux.target)-1);
	}
	else
	{
		p = json_get_string(json_result, "_target");
		if(p && strlen(p) >= 64)
		{
			char target_be[65];
			memset(target_be, 0, sizeof(target_be));
			// "_target" is documented as reversed byte order.
			string_be(p, target_be);
			strncpy(coind->aux.target, target_be, sizeof(coind->aux.target)-1);
		}
		else
		{
			...
			{
				char target_hex[65];
				memset(target_hex, 0, sizeof(target_hex));
				if(auxpow_target_from_bits_be(bits, target_hex, sizeof(target_hex)))
					strncpy(coind->aux.target, target_hex, sizeof(coind->aux.target)-1);
			}
		}
	}
...
YAAMP_JOB_TEMPLATE *coind_create_template(YAAMP_COIND *coind)
{
	...
	// GBT rules selection (per-coin):
	//  usemweb=1   -> ["segwit","mweb"]
	//  usesegwit=1 -> ["segwit"]
	//  else        -> no rules requested
	else if(coind->usemweb) strcpy(params, "[{\"rules\":[\"segwit\",\"mweb\"]}]");
	...
	if(json_rules && !strlen(coind->witness_magic) && json_rules->u.array.length) {
		for (int i=0; i<json_rules->u.array.length; i++) {
			...
			if(!strcmp(val->u.string.ptr, "segwit")) {
				...
				// usemweb implies segwit
				if (coind->usemweb) coind->usesegwit = true;
				if (coind->usesegwit)
