...
void compact_to_target_be(const char *nbits_hex, unsigned char target_be[32])
{
	memset(target_be, 0, 32);
	if(!nbits_hex || !nbits_hex[0]) return;

	uint32_t nBits = (uint32_t)htoi64(nbits_hex);
	...
	if(exp <= 3)
	{
		mant >>= 8 * (3 - exp);
		for(int i=0; i<exp; i++)
			target_be[32 - exp + i] = (mant >> (8 * (exp - 1 - i))) & 0xff;
		return;
	}

	int start = 32 - exp;
	if(start < 0) start = 0;
	if(start + 2 >= 32) return;

	target_be[start + 0] = (mant >> 16) & 0xff;
	target_be[start + 1] = (mant >> 8) & 0xff;
	target_be[start + 2] = mant & 0xff;
}

bool be256_leq(const unsigned char a_be[32], const unsigned char b_be[32])
{
	for(int i=0; i<32; i++)
	{
		if(a_be[i] < b_be[i]) return true;
		if(a_be[i] > b_be[i]) return false;
	}
	return true;
}

void reverse32(const unsigned char in[32], unsigned char out[32])
{
	for(int i=0; i<32; i++)
		out[i] = in[31-i];
}

bool le256_leq(const unsigned char a_le[32], const unsigned char b_le[32])
{
	// Compare from most significant byte (index 31) down to least (index 0).
	for(int i=31; i>=0; i--)
	{
		if(a_le[i] < b_le[i]) return true;
		if(a_le[i] > b_le[i]) return false;
	}
	return true;
}
