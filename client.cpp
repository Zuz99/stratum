...
bool client_subscribe(YAAMP_CLIENT *client, json_value *json_params)
{
	...
	// VARDIFF: start new connections at diff_min and let it increase with accepted shares
	client->difficulty_actual = g_stratum_min_diff;
	...
	if(json_params->u.array.length>0)
	{
		...
		if (strstr(client->version, "NiceHash"))
			client->difficulty_actual = g_stratum_nicehash_min_diff;
...
//	client->source = source_init(client);
	...
	client->last_vardiff_time = client->last_submit_time;
